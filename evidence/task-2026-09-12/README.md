...
## Review repair 2 (EN-R03S-b, 2026-09-13)
Review 2 reproduced: replaying a saved snapshot into its own run directory deleted the snapshot CSV before copying it and raised `FileNotFoundError`, leaving the old report pointing at a missing file; and metadata could be changed between snapshot and intake so the intake classified a "physical" declaration while metrics used the original synthetic dictionary. Both reproduced here.
Fix: `snapshot()` refuses (`CollisionError`, CLI exit 5) **before touching anything** if an input is a saved snapshot inside the output directory or if the output directory already holds a rehearsal; snapshot copies are written from bytes read once and both CSV **and metadata bytes** are what intake and metrics consume; the intake's `metadata_sha256` must equal the metrics' metadata or `IntegrityError` is raised; the report records `metadata_input_sha256`. Five new tests, including replaying a snapshot into a fresh directory reproducing the report exactly. 26 rehearsal tests; discover with the same two sandbox-only failures.
